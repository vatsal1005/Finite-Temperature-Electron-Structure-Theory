...
##---------------------------------------------------------------------------##
  ...
  if calc == 'CCD':
    ...
    R_ijab = amplitude.doubles(I_oo,I_vv,Ivvvv,Ioooo,Iovvo,Iovvo_2,Iovov,Iovov_2,t2)
    #R_ijab = amplitude.symmetrize(R_ijab)
    R_ijab = cc_symmetrize.symmetrize(R_ijab)
    oldt2 = t2.copy()
    #eps_t, t2 = amplitude.update_t2(R_ijab,t2)
    eps_t, t2, R_ijab = cc_update.update_t2(R_ijab,t2)
    if inp.diis == True:
      if x+1>max_diis:
        # Limit size of DIIS vector
        if (len(diis_vals_t2) > max_diis):
...
##---------------------------------------------------------------------------##
  ...
  if calc == 'CCSD':
    ...
    R_ijab = amplitude.doubles(I_oo,I_vv,Ivvvv,Ioooo,Iovvo,Iovvo_2,Iovov,Iovov_2,tau,t2,t1)
